...
function badge(type){if(type==='FK')return'<span class="badge badge-fk">FK</span>';if(type==='NAMING_PATTERN')return'<span class="badge badge-naming">명명</span>';if(type==='DATA_SIMILAR')return'<span class="badge badge-value">값유사</span>';if(type==='EXACT_MATCH')return'<span class="badge" style="background:#4caf50;color:#fff">정확</span>';if(type==='NORMALIZED')return'<span class="badge" style="background:#2196f3;color:#fff">정규화</span>';if(type==='SYNONYM')return'<span class="badge" style="background:#ff9800;color:#fff">동의어</span>';if(type==='SEMANTIC')return'<span class="badge" style="background:#9c27b0;color:#fff">의미</span>';if(type==='SIMILAR')return'<span class="badge" style="background:#9c27b0;color:#fff">유사</span>';if(type==='COMMENT_MATCH')return'<span class="badge" style="background:#2196f3;color:#fff">코멘트</span>';return`<span class="badge" style="background:#eee">${type}</span>`}
...
function toggleSortComment(){sortCommentOnly=!sortCommentOnly;if(sortCommentOnly)sortSelected=false;renderPgTables()}
function toggleSortSelected(){sortSelected=!sortSelected;if(sortSelected)sortCommentOnly=false;renderPgTables()}
...
async function loadGraph(){const container=document.getElementById('graph-container');container.innerHTML='<p style="color:#999;padding:20px">그래프 로딩 중...</p>';const data=await getJSON('/api/graph');if(!data||!data.nodes||!data.nodes.length){container.innerHTML='<p style="color:#999;padding:20px">그래프 데이터가 없습니다. 먼저 분석을 실행하세요.</p>';return}container.innerHTML='';const relTypes=new Set(['FK','EXACT_MATCH','NORMALIZED','SYNONYM','SEMANTIC','SIMILAR','DATA_SIMILAR','NAMING_PATTERN','COMMENT_MATCH']);const degree={};const nodeMap={};data.nodes.forEach(n=>{nodeMap[n.id]=n});data.links.forEach(lk=>{if(relTypes.has(lk.type)){const s=typeof lk.source==='object'?lk.source.id:lk.source;const t=typeof lk.target==='object'?lk.target.id:lk.target;degree[s]=(degree[s]||0)+1;degree[t]=(degree[t]||0)+1}});let maxTC=0;data.nodes.forEach(n=>{if(n.group==='COLUMN'&&n.table_count>maxTC)maxTC=n.table_count});const colRadius=tc=>Math.min(28,8+tc*4);const colColor=tc=>{if(!tc)return'#ffb74d';const t=maxTC>1?Math.min(tc/maxTC,1):1;const r=Math.round(255+(183-255)*t);const g=Math.round(183+(28-183)*t);const b=Math.round(77+(28-77)*t);return`rgb(${r},${g},${b})`};let maxDeg=0;Object.values(degree).forEach(d=>{if(d>maxDeg)maxDeg=d});const degRadius=deg=>!deg?6:6+(maxDeg>0?(deg/maxDeg)*14:0);const w=container.clientWidth||800,h=500;const svg=d3.select(container).append('svg').attr('width',w).attr('height',h);const g=svg.append('g');svg.on('click',()=>{const el=document.getElementById('graphRelationDetail');if(el)el.style.display='none'});const zoom=d3.zoom().scaleExtent([.1,4]).on('zoom',e=>g.attr('transform',e.transform));svg.call(zoom);const sim=d3.forceSimulation(data.nodes).force('link',d3.forceLink(data.links).id(d=>d.id).distance(80)).force('charge',d3.forceManyBody().strength(-200)).force('center',d3.forceCenter(w/2,h/2)).force('collision',d3.forceCollide(25));const link=g.selectAll('.link').data(data.links).enter().append('line').attr('class','link').attr('stroke-width',d=>Math.sqrt(d.value||1));const node=g.selectAll('.node').data(data.nodes).enter().append('g').attr('class','node').call(d3.drag().on('start',(e,d)=>{if(!e.active)sim.alphaTarget(.3).restart();d.fx=d.x;d.fy=d.y}).on('drag',(e,d)=>{d.fx=e.x;d.fy=e.y}).on('end',(e,d)=>{if(!e.active)sim.alphaTarget(0);d.fx=null;d.fy=null})).on('click',function(e,d){if(d.group!=='COLUMN')return;e.stopPropagation();const el=document.getElementById('graphRelationDetail');const tables=[];data.links.forEach(lk=>{if(lk.type!=='HAS_FIELD')return;const tgt=typeof lk.target==='object'?lk.target.id:lk.target;if(tgt===d.id){const src=typeof lk.source==='object'?lk.source.id:lk.source;const srcNode=nodeMap[src];if(srcNode&&srcNode.group==='TABLE')tables.push(srcNode)}});const rels=[];data.links.forEach(lk=>{if(!relTypes.has(lk.type))return;const s=typeof lk.source==='object'?lk.source.id:lk.source;const t=typeof lk.target==='object'?lk.target.id:lk.target;if(s===d.id||t===d.id)rels.push({lk,s,t})});let html='<div style="background:#f8f9fa;border-radius:8px;padding:12px;margin-top:12px;border:1px solid #e0e0e0"><div style="font-weight:600;font-size:14px;margin-bottom:8px;color:#333">📎 <strong>'+d.name+'</strong><span style="font-weight:normal;font-size:12px;color:#888"> · '+d.table_count+'개 테이블</span></div>';if(tables.length){html+='<table style="font-size:12px;width:100%"><tr><th style="text-align:left;padding:4px 8px;background:#eee">DB</th><th style="text-align:left;padding:4px 8px;background:#eee">스키마</th><th style="text-align:left;padding:4px 8px;background:#eee">테이블</th></tr>';tables.forEach(t=>{let schemaNode=null,dbNode=null;data.links.forEach(lk=>{if(lk.type==='HAS_TABLE'){const tgt=typeof lk.target==='object'?lk.target.id:lk.target;if(tgt===t.id){const src=typeof lk.source==='object'?lk.source.id:lk.source;schemaNode=nodeMap[src]}}if(lk.type==='HAS_SCHEMA'){const tgt=typeof lk.target==='object'?lk.target.id:lk.target;if(schemaNode&&tgt===schemaNode.id){const src=typeof lk.source==='object'?lk.source.id:lk.source;dbNode=nodeMap[src]}}});html+='<tr><td style="padding:4px 8px">'+(dbNode?dbNode.name:'')+'</td><td style="padding:4px 8px">'+(schemaNode?schemaNode.name:'')+'</td><td style="padding:4px 8px"><strong>'+t.name+'</strong></td></tr>'});html+='</table>'}if(rels.length){html+='<h4 style="font-size:13px;margin:12px 0 4px;color:#555">관계</h4><table style="font-size:12px;width:100%"><tr><th style="text-align:left;padding:4px 8px;background:#eee">유형</th><th style="text-align:left;padding:4px 8px;background:#eee">관계</th><th style="text-align:left;padding:4px 8px;background:#eee">신뢰도</th></tr>';rels.forEach(r=>{const srcN=nodeMap[r.s];const tgtN=nodeMap[r.t];const sl=srcN?srcN.name:r.s;const tl=tgtN?tgtN.name:r.t;html+='<tr><td style="padding:4px 8px">'+badge(r.lk.type)+'</td><td style="padding:4px 8px;font-size:11px"><span style="color:#555">'+sl+'</span><span style="color:#999;margin:0 4px">→</span><span style="color:#555">'+tl+'</span></td><td style="padding:4px 8px">'+(r.lk.confidence*100).toFixed(0)+'%</td></tr>'});html+='</table>'}if(!tables.length&&!rels.length)html+='<p style="color:#999;padding:8px">연결된 정보가 없습니다.</p>';html+='</div>';el.innerHTML=html;el.style.display='block'});const rectPath=(w,h,r)=>{const rr=Math.min(r,w/2,h/2);return'M'+(-w/2+rr)+','+(-h/2)+'a'+rr+','+rr+',0,0,0,'+(-rr)+','+rr+'v'+(h-2*rr)+'a'+rr+','+rr+',0,0,0,'+rr+','+rr+'h'+(w-2*rr)+'a'+rr+','+rr+',0,0,0,'+rr+','+(-rr)+'v'+(-h+2*rr)+'a'+rr+','+rr+',0,0,0,'+(-rr)+','+(-rr)+'z'};const sym=d3.symbol();node.append('path').attr('d',d=>{if(d.group==='TABLE'){const ww=Math.max(50,d.name.length*8+16);return rectPath(ww,22,5)}if(d.group==='COLUMN'){const r=colRadius(d.table_count||1);sym.type(d3.symbolCircle).size(Math.pow(r*2,2));return sym()}if(d.group==='SCHEMA'||d.group===2){return d3.symbol().type(d3.symbolDiamond).size(400)()}if(d.group==='DOMAIN'){return d3.symbol().type(d3.symbolTriangle).size(360)()}sym.type(d3.symbolCircle).size(d.group==='DATABASE'||d.group===1?1296:144);return sym()}).attr('fill',d=>{if(d.group==='COLUMN')return colColor(d.table_count||0);return d.color||'#888'}).style('cursor',d=>d.group==='COLUMN'?'pointer':'default');node.append('text').text(d=>d.name||d.label||d.id).attr('dx',d=>{if(d.group==='TABLE'){const ww=Math.max(50,d.name.length*8+16);return ww/2+5}if(d.group==='COLUMN'){return colRadius(d.table_count||1)+5}return 15}).attr('dy',4).style('font-size','10px');sim.on('tick',()=>{link.attr('x1',d=>d.source.x).attr('y1',d=>d.source.y).attr('x2',d=>d.target.x).attr('y2',d=>d.target.y);node.attr('transform',d=>`translate(${d.x},${d.y})`)});container.insertAdjacentHTML('afterend','<div id="graphRelationDetail" style="margin-top:12px;display:none"></div>')}async function doSearch(q){if(q.length<2){document.getElementById('searchResults').innerHTML='<p style="color:#999">2글자 이상 입력하세요</p>';return}const d=await getJSON('/api/search?q='+encodeURIComponent(q));if(!d){document.getElementById('searchResults').innerHTML='<p style="color:#999">오류</p>';return}let html='';if(d.tables&&d.tables.length){html+='<h3 style="font-size:14px;margin:8px 0">테이블</h3><table>';d.tables.forEach(t=>{html+=`<tr><td><strong>${t.name}</strong></td><td>${t.description||''}</td></tr>`});html+='</table>'}if(d.columns&&d.columns.length){html+='<h3 style="font-size:14px;margin:8px 0">컬럼</h3><table>';d.columns.forEach(c=>{html+=`<tr><td>${c.name}</td><td>${c.type||''}</td><td>${c.table}</td><td>${c.description||''}</td></tr>`});html+='</table>'}if(!html)html='<p style="color:#999">검색 결과 없음</p>';document.getElementById('searchResults').innerHTML=html}
